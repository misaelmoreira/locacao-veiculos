class LojaController < UsuariosController
  include HTTParty
  #skip_before_action :verify_authenticity_token, only: %i[alugar finalizar_pagamento]
  before_action :set_veiculo, only: %i[aluguel alugar]
  ...
  def alugar
    if params[:dias].present? && params[:token].present? && params[:senderHash].present?
      ...
      if cookies[:usuario].present?
        reserva = Reserva.new(reservado_de: Time.now, reservado_ate: Time.now + dias.days,
                              valor_alugado: @valor_total, status: Reserva::STATUS[:aguardando], veiculo_id: @veiculo.id, usuario_id: cookies[:usuario])
        if reserva.save!
  ...
  def confirmacao_pagamento
    return unless params[:token_pagamento].present? && params[:hash_comprador].present?
  ...
  def finalizar_pagamento
    if params[:reserva_id].present? && params[:token_pagamento].present? && params[:hash_comprador].present?
      ...
      body = {
        ...
        creditCardHolderPhone: 15_999_999_999,
        itemId1: veiculo.id,
      ...
      if response.code == 200
        ...
        redirect_to '/finalizado?reserva_id='
      end
